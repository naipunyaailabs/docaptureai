...
export async function runRfpAgent({ documentText, systemPromptOverride }: RfpAgentOptions): Promise<RfpAgentResult> {
  ...
  try {
    console.log("[RFP Agent] Sending request to AI API with document length:", documentText.length);
    raw = await unifiedChatCompletion(systemPrompt, prompt);
    console.log("[RFP Agent] Received response from AI API with raw length:", raw.length);
